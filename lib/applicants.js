...
const { Promise, co, omit, collect, poster, getter, putter, authRequest, errorFromResponse } = utils
...
module.exports = function createApplicantsAPI ({ token }) {
  ...
  const put = putter(token)
  const auth = authRequest(token)
  const create = co(function* (applicantData) {
    typeforce({
      first_name: typeforce.String,
      last_name: typeforce.String,
      // email: typeforce.String // only required for US and UK checks
      // TODO: optional props
    }, applicantData)
  ...
  const update = co(function* (applicantId, applicantData) {
  ...
  const get = co(function* (applicantId) {
    typeforce(typeforce.String, applicantId)
  ...
  const uploadDocument = co(function* (applicantId, doc) {
    typeforce(typeforce.String, applicantId)
    typeforce({
      type: types.docType,
  ...
  const uploadLivePhoto = co(function* (applicantId, photo) {
    typeforce(typeforce.String, applicantId)
  ...
  const deleteApplicant = co(function* (applicantId) {
    typeforce(typeforce.String, applicantId)
